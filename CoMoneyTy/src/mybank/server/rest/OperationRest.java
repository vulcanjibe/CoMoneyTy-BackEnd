...
public class OperationRest {
    ...
    public Response get(@Context HttpHeaders headers, @Context UriInfo uriInfo) {
        ...
        try {
            // Vérification de l'accès depuis un operation connecté
            connexionUser = ConnexionUser.verificationConnexionUser(headers);
            List<Operation> liste = (List<Operation>) AccesseurGenerique.getInstance().getListe(Operation.class);
    ...
    public Response getById(@Context HttpHeaders headers, @Context UriInfo uriInfo, @PathParam("id") String strid) {
        ...
        try {
            // Vérification de l'accès depuis un operation connecté
            connexionUser = ConnexionUser.verificationConnexionUser(headers);
            Operation aOperation= (Operation) AccesseurGenerique.getInstance().get(Operation.class, strid);
            if(aOperation==null)
                throw new Exception("User inconnu");
            // Traitement de la log
            Utilitaire.loggingRest(this.getClass(), "getById", strid, connexionUser);
            return Reponse.getResponseOK(aOperation);
        } catch (Exception e) {
            // Traitement de l'exception
    ...
    @Path("/{id}/document")
    @Produces(MediaType.APPLICATION_JSON)
    public Response getDocuments(@Context HttpHeaders headers, @Context UriInfo uriInfo, @PathParam("id") String strid) {
        ConnexionUser connexionUser = null;
        try {
            // Vérification de l'accès depuis un operation connecté
            connexionUser = ConnexionUser.verificationConnexionUser(headers);
            List<Document> documents = AccesseurGenerique.getInstance().getListeFiltre(Document.class, " idOperation='"+strid+"'");
            // Traitement de la log
            Utilitaire.loggingRest(this.getClass(), "getById", strid, connexionUser);
            return Reponse.getResponseOK(documents);
        } catch (Exception e) {
            // Traitement de l'exception
            Utilitaire.exceptionRest(e, this.getClass(), "getById", strid, connexionUser);
            return Reponse.reponseKO(e);
   ...
   //     mapper.setDateFormat(Utilitaire.FORMAT_DATE_COURT);
        ...
        try {
            // Vérification de l'accès depuis un operation connecté
            connexionUser = ConnexionUser.verificationConnexionUser(headers);
            Operation aOperation= mapper.readValue(new String(data, "UTF-8"), Operation.class);
            // Traitement de la photo
          
            
           	AccesseurGenerique.getInstance().save(aOperation);
    ...
    public Response delete(@Context HttpHeaders headers, @Context UriInfo uriInfo,  @PathParam("id") String strid) {
        ...
        try {
            ...
            aOperation.setId(strid);
            AccesseurGenerique.getInstance().delete(aOperation);
            // Traitement de la log
            Utilitaire.loggingRest(this.getClass(), "delete", "operation/"+strid, connexionUser);
            return Reponse.getResponseOK(aOperation);
        } catch (Exception e) {
            // Traitement de l'exception
            Utilitaire.exceptionRest(e, this.getClass(), "delete","operation/"+strid, connexionUser);
            return Reponse.reponseKO(e);
        }
    }
